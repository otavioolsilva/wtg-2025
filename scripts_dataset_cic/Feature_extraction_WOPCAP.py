...
from Communication_features import Communication_wifi
...
class Feature_extraction():
    ...
    def pcap_evaluation(self,pcap,csv_file_name,mode):
        ...
        processed_df.to_csv(csv_file_name+".csv", index=False, header=mode=='w', mode=mode)
        return True

    def pcap_evaluation_caller(self,args):
        mode = 'w'

        while True:
            job = args.sem_full.acquire(timeout=1)
            if not job:
                if args.stop.is_set():
                    break
                continue

            self.pcap_evaluation(args.buf[args.front], args.file_name, mode)
            args.front = (args.front + 1) % args.buf_size
            mode = 'a'

            args.sem_empty.release()
